...
class SymbolManager:
    def __init__(self, api, symbols_directory="symbols"):
        ...
        self.index_specs = {
            "NIFTY": {"symbol_prefix": "NIFTY", "lot_size": 65},
            "BANKNIFTY": {"symbol_prefix": "BANKNIFTY", "lot_size": 30},
            "FINNIFTY": {"symbol_prefix": "FINNIFTY", "lot_size": 60},
        }
    ...
    def _save_master_file(self, df, name):
        ...
        if df is not None:
            try:
                ...
                filename = os.path.join(self.master_directory, f"{name}_master_{datetime.now().strftime('%Y%m%d')}.csv")
    ...
    def get_token_info(self, symbol, exchange="NSE"):
        ...
        try:
            ...
            if len(symbol_match) == 0 and "tradingsymbol" in df.columns:
                symbol_match = df[(df["tradingsymbol"].str.lower() == symbol.lower())]
        ...
        try:
            ...
            for index in indices:
                try:
                    ...
                    self.logger.info(f"{index}: Future price = {current_price:.2f}, ATM strike = {atm_strike:.2f}")
                    ...
                    selected_options = index_options[
                        (index_options["strikeprice"] >= min_strike) & (index_options["strikeprice"] <= max_strike)
                    ]
                    ...
                    for _, option in selected_options.iterrows():
                        ...
                        self.logger.debug(
                            f"Added {index} {option['optiontype']} @ {option['strikeprice']} (Token: {option['token']})"
                        )
    ...
    def get_all_index_derivatives(self, max_expiries_per_index=2):
        ...
        for index in ["NIFTY", "BANKNIFTY", "FINNIFTY"]:
            ...
            today = datetime.now().date()
            index_options_all = index_options_all[index_options_all["expiry_date"].dt.date >= today]

            if index_options_all.empty:
                continue

            # Get unique expiries, sorted
            unique_expiries = sorted(index_options_all["expiry_date"].dt.date.unique())[:max_expiries_per_index]
            ...
            for stock in self.stock_symbols:
                try:
                    ...
                    selected_options = stock_options[
                        (stock_options["strikeprice"] >= min_strike) & (stock_options["strikeprice"] <= max_strike)
                    ]
                    ...
                    for _, option in selected_options.iterrows():
                        ...
                        self.logger.debug(f"Added {stock} {option['optiontype']} @ {option['strikeprice']}")
    ...
    def scan_common_etfs(self):
        ...
        try:
            ...
            # Find ETFs in NSE
            nse_etfs = self.nse_cash[self.nse_cash["symbol"].str.contains("ETF|BEES", case=False, na=False)]

            # Find ETFs in BSE
            bse_etfs = self.bse[self.bse["symbol"].str.contains("ETF|BEES", case=False, na=False)]
