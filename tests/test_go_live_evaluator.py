...
def _clean_report(date: str, pairs: int = 4):
    ...
    return {
        ...
        "flagged_count": 0,
        "matched": [{"symbol": f"LEG{i}", "side": "SELL", "price_delta_pct": 0.0} for i in range(pairs)],
        "unmatched_engine": [],
        "unmatched_broker": [],
        "flagged": [],
    }
...
def _perfect_reports():
    ...
    return [_clean_report(f"2026-03-{d + 1:02d}") for d in range(settings.GL_MIN_RECONCILED_DAYS)]
...
def test_total_matches_checks_count():
    ...
    assert res["total"] == len(res["checks"]), "total must equal the number of checks performed"
