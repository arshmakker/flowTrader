...
# ── ENTRY_CUTOFF: no new entries after 14:30 ─────────────────────────────────


def test_entry_cutoff_blocks_entry_at_or_after_cutoff():
    """Bug B regression: entry logic must not call s.enter() at or after ENTRY_CUTOFF.
    A position opened at 14:46 carried overnight when the next day was a trading day,
    producing a ₹43k gap loss. The fix gates the entry block on now_t < ENTRY_CUTOFF."""
    from datetime import time
    from unittest.mock import MagicMock, patch

    from trading_system.config import settings

    class _EntryCountingStrategy:
    ...
    s = _EntryCountingStrategy()
    risk = MagicMock()
    risk.halted = False

    # Simulate now_t = 14:46, ENTRY_CUTOFF = "14:30"
    after_cutoff = time(14, 46)

    with patch.object(settings, "ENTRY_CUTOFF", "14:30"):
        # Manually replicate the gate that main.py applies:
        # if not risk.halted and now_t < _entry_cutoff_t: enter()
        cutoff_t = __import__("datetime").datetime.strptime(settings.ENTRY_CUTOFF, "%H:%M").time()
        if not risk.halted and after_cutoff < cutoff_t:
            s.enter()

    assert not s.entered, "enter() must not be called at 14:46 when ENTRY_CUTOFF=14:30"


def test_entry_cutoff_allows_entry_before_cutoff():
    """Verify entries are still allowed before ENTRY_CUTOFF."""
    from datetime import time
    from unittest.mock import MagicMock, patch
    ...
    with patch.object(settings, "ENTRY_CUTOFF", "14:30"):
        cutoff_t = __import__("datetime").datetime.strptime(settings.ENTRY_CUTOFF, "%H:%M").time()
        if not risk.halted and before_cutoff < cutoff_t:
            s.enter()

    assert s.entered, "enter() must be called before ENTRY_CUTOFF"
