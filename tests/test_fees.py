...
def test_stamp_buy_rate():
    ...
    f = compute_taxes_and_fees(_CE, "BUY", price=50.0, qty=650)
    expected = 50.0 * 650 * 0.00003  # ₹0.98
...
def test_worked_example_10lot_sell_at_50():
    ...
    Component-by-component against the documented LIVE-12 design numbers:
      brokerage   =   5.00
      stt         = 32500 × 0.0015    = 48.75
      exch_txn    = 32500 × 0.0003553 = 11.55
      sebi        = 32500 × 0.000001  =  0.03
      stamp       = 0 (SELL)
      gst         = (5 + 11.55 + 0.03) × 0.18 = 2.98
      total       ≈ 68.31
    """
    f = compute_taxes_and_fees(_CE, "SELL", price=50.0, qty=650)
    assert f["brokerage"] == pytest.approx(5.00, abs=0.01)
    assert f["stt"] == pytest.approx(48.75, abs=0.01)
    assert f["exch_txn"] == pytest.approx(11.55, abs=0.01)
    assert f["sebi"] == pytest.approx(0.03, abs=0.01)
    assert f["stamp"] == 0.0
    assert f["gst"] == pytest.approx(2.98, abs=0.01)
    assert f["total"] == pytest.approx(68.31, abs=0.02)


def test_worked_example_10lot_buy_at_5_is_cheap():
    """10 lots NIFTY BUY CE @ ₹5 (short exit at harvest). Turnover ₹3,250 —
    ₹25-ish all-in. Sanity-check that BUY-side cost stack is materially
    ...
    assert f["stt"] == 0.0
    assert f["exch_txn"] == pytest.approx(1.15, abs=0.01)
    assert f["sebi"] == pytest.approx(0.0, abs=0.01)
    assert f["stamp"] == pytest.approx(0.10, abs=0.01)
    assert f["gst"] == pytest.approx((5 + 1.15 + 0.0) * 0.18, abs=0.02)
    # All-in ~₹7.36 — meaningfully smaller than SELL of same turnover.
...
def test_put_symbol_recognised_as_option():
    """PE tradingsymbols ended in 'P{strike}' must go through the options
...
def test_rates_block_is_current_as_of_2026_04_24():
    ...
    assert r["exch_txn_pct"] == 0.0003553
