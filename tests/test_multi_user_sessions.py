...
print(Fore.BLUE + str(ret2))

# Individual Websocket callbacks for each account.
def user1_socket_order_update(message):
    print(Fore.YELLOW + f"OrderInfo Received for User1: {message}")

def user1_socket_quote_update(message):
    print(Fore.YELLOW + f"Quote Received for User1: {message}")

def user1_socket_open():
    print(Fore.CYAN + "Websocket is now open for User1:")
    user1.subscribe('NSE|26000')

def user2_socket_order_update(message):
    print(Fore.MAGENTA + f"OrderInfo Received for User2: {message}")

def user2_socket_quote_update(message):
    print(Fore.MAGENTA + f"Quote Received for User2: {message}")

def user2_socket_open():
    print(Fore.CYAN + "Websocket is now open for User2:")
...
print(Fore.GREEN + "Both accounts are initialized, now the quotes/order updates should come for both users.")
