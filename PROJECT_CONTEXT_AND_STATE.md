...
**Convex backspread backtest (two-fork)** (2026-02-08): **`backtest_convex_backspread.py`** uses production regime (TRENDING/SIDEWAYS). Convex **entries** when regime is **TRENDING** only. Exit: regime flip to SIDEWAYS (after confirmation), time 40%, ATR, re-compression, max loss, TSL. **Backtest-only relaxations**: (1) net debit cap override via `max_net_debit_pct=0.007` (0.7% of spot); (2) when the option chain has only one CE strike, a synthetic OTM strike is added. **Entry/exit tweaks (backtest + production)**: (3) no new Convex entry within 45 min of market close (15:30); (4) TIME_ELAPSED_40PCT exit only when in loss (don’t time-exit when in profit). On start, backtest logs **data available** range from `market_data_*` dirs. Report includes `avg_pnl` and **entry_diagnostics**. **Pattern analysis**: Each saved trade in the report includes **entry_spot**, **iv_percentile_at_entry**, **adx_at_entry**, **entry_range_state**, **entry_hour**, **hold_minutes** (from existing backtest data only). **`analyze_convex_backtest.py`** loads report JSON(s), groups by exit_reason, entry_hour, entry_debit bins, hold_minutes, IV/ADX bins, and prints count, total/avg P&L, win rate per segment (no live market connection). **Note**: Backtest P&L uses a simplified option-pricing proxy; live results can differ.
